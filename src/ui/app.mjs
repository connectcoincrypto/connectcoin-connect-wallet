...
function diagnosticPanel() {
  ...
  return `<section class="card diagnostics-card" aria-label="Local diagnostics"><div class="card-top"><h2>Recent diagnostic events</h2><button class="text-button" data-action="open-diagnostics">Open log folder ${icon('external')}</button></div><p class="card-description">This is a history of past errors and individual attempt failures, not the current status of Automatic Claims. Up to 50 events from this app session remain here after recovery. Full diagnostics are saved locally, with UTC timestamps and automatic file rotation. No recovery words, passwords, addresses or transaction contents are logged.</p>${diagnostics.status === 'unavailable' ? notice('The diagnostic log could not be written. Recent errors are available in memory only; check folder permissions and free disk space.', 'danger') : ''}${diagnostics.dropped ? notice(`${e(diagnostics.dropped)} diagnostic records could not be saved. The history may be incomplete.`) : ''}<p class="diagnostic-path">${e(diagnostics.file ?? '')}</p>${recent.length ? `<ol class="diagnostic-list">${recent.map(row => {
    const details = row.details ?? {}, error = details.error ?? {};
    const connectionTimeout = row.event === 'claim.failed' && details.stage === 'proof' && error.category === 'tls-timeout';
    const metadata = [connectionTimeout ? 'TCP/TLS connection' : details.stage, details.method, details.claimId != null ? `bounty job #${details.claimId}` : null, error.code != null ? `RPC/OS ${error.code}` : null, error.nodeCode != null ? `node ${error.nodeCode}` : null, Number.isFinite(details.durationMs) ? `${(details.durationMs / 1000).toFixed(3)} s` : null].filter(value => value != null);
    return `<li><div class="diagnostic-meta"><time>${e(row.timestamp)}</time><span>${e(row.event)}</span></div><strong>${e(connectionTimeout ? 'TCP/TLS connection attempt timed out' : error.message ?? 'Operation failed; see the diagnostic log.')}</strong>${connectionTimeout ? `<p class="diagnostic-explanation">${e(error.message)} This individual timeout did not stop Automatic Claims.</p>` : ''}<p>${e(metadata.join(' · '))}</p></li>`;
...
function developerSettings() {
  ...
  return `<section class="card form-card"><div class="setting-row"><div><h2>Developer Mode</h2><p id="developer-mode-description">Show recoverable claim rejection warnings and the Recent diagnostic events panel in Automatic claims.</p></div><button id="developer-mode" class="switch ${enabled ? 'on' : ''}" role="switch" aria-checked="${enabled}" aria-label="Developer Mode" aria-describedby="developer-mode-description" data-action="toggle-developer-mode" data-busy><span></span></button></div><p class="field-help">Off by default. Local diagnostic logging stays active in either mode. Important connection, security and stopped-claim alerts remain visible.</p></section>`;
}
